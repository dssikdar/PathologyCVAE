...
class UNetEncoder(nn.Module):
    def __init__(self, in_channels=3, latent_dim=28, freeze_encoder=True):
        ...
        self.fc_mu = nn.Linear(512 * (target_image_size // 32) * (target_image_size // 32), latent_dim)
        self.fc_logvar = nn.Linear(512 * (target_image_size // 32) * (target_image_size // 32), latent_dim)
        if freeze_encoder:
...
if __name__ == '__main__':
    ...
    if use_subset:
        subset_size = 1000  # Reduce dataset size for testing
    ...
    vae = VAE(latent_dim=96, freeze_encoder=True).to(device)
    ...
    encoder_path = os.path.join(output_path, "base_encoder.pth")

    # Save only the encoder's state_dict
    torch.save(vae.state_dict(), encoder_path)
